...
if __name__ == "__main__":
    ...
    # load the expert samplers from a file
    ...
    replay_buffer = StepReplayBuffer(max_num_steps=args.max_num_samples)
    ...
    for i, path in enumerate(x[:args.max_num_samples]):
        observations = []
        actions = []
        rewards = []

        for t in range(min(path["observation"].shape[0], args.max_num_samples)):
            observations.append({
                "observation": (
                    path["observation"][t, ...] - observation_low) * 2.0 / (
                        observation_high - observation_low) - 1.0})
            actions.append((
                path["action"][t, ...] - action_low) * 2.0 / (
                    action_high - action_low) - 1.0)
            rewards.append(path["reward"][t, ...])

        # insert entire paths into the buffer
        replay_buffer.insert_path(observations, actions, rewards)

    # save the replay buffer to a file
    replay_buffer.save(args.replay_buffer_file)
